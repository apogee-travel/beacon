...
type ActorActionParameters<T extends (...args: any[]) => any> =
  T extends (actor: any, ...args: infer P) => any ? P : never;
```
...
const toggleMachine = setup({
  ...
  states: {
    inactive: { on: { TOGGLE: { target: "active", actions: [({ context }) => { context.enabled = true; }] } } },
    active: {
      on: {
        TOGGLE: { target: "inactive", actions: [({ context }) => { context.enabled = false; }] },
        INCREMENT: { actions: [({ context }) => { context.count++; }] },
      },
    },
  },
});
...
function defineActorActions<
  TMachine extends AnyActorLogic,
  TActorActions extends Record<string, ActorAction<TMachine, any>> = Record<string, ActorAction<TMachine, any>>,
